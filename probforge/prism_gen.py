...
from forge_core.ir import RuleSet, Variable
from forge_core.schema import check_finite_state


class ProbForgeError(Exception):
...
def _var_decl(v: Variable) -> str:
    ...
    raise ProbForgeError(f"PRISM 백엔드가 지원하지 않는 타입: '{v.name}'({v.type})")
...
def _commands(ruleset: RuleSet) -> list[str]:
    ...
    for t in ruleset.transitions:
        ...
        if total <= 0:
            raise ProbForgeError(f"전이 '{t.id}'의 weight 합이 0입니다.")
...
def _properties(ruleset: RuleSet) -> tuple[PrismProperty, ...]:
    ...
    for p in ruleset.properties:
        ...
        if p.kind == "reachable":
            ...
            if not p.spec:
                raise ProbForgeError(f"속성 '{p.id}'(prob)에 spec(PCTL)이 없습니다.")
            pctl = p.spec  # ProbForge 전용 원문(D11)
        else:
            raise ProbForgeError(f"속성 '{p.id}'의 알 수 없는 kind: '{p.kind}'")
...
def _render(node: ast.AST) -> str:
    ...
    if isinstance(node, ast.UnaryOp):
        ...
        if isinstance(node.op, ast.USub):
            return f"-{_render(node.operand)}"
        raise ProbForgeError("지원하지 않는 단항 연산자")
    ...
    if isinstance(node, ast.Constant):
        ...
        raise ProbForgeError(f"PRISM 백엔드가 지원하지 않는 상수: {node.value!r}")
    if isinstance(node, ast.Attribute):
        raise ProbForgeError("next.* 는 전이 갱신(then)에서만 쓸 수 있습니다.")
    raise ProbForgeError(f"PRISM 백엔드가 지원하지 않는 표현식 요소: {type(node).__name__}")
...
def _render_compare(node: ast.Compare) -> str:
    ...
    for i, op in enumerate(node.ops):
        ...
        if sym is None:
            raise ProbForgeError(f"지원하지 않는 비교 연산자: {type(op).__name__}")
...
def _assignment(node: ast.AST) -> tuple[str, ast.expr]:
    """`next.X == 식`에서 (X, 식 노드)를 뽑는다. 배정형이 아니면 ProbForgeError."""
    if not (
        isinstance(node, ast.Compare) and len(node.ops) == 1 and isinstance(node.ops[0], ast.Eq)
    ):
        raise ProbForgeError("PRISM 갱신은 'next.X == 식' 배정형이어야 합니다(부등식 등 불가).")
    for a, b in ((node.left, node.comparators[0]), (node.comparators[0], node.left)):
        if isinstance(a, ast.Attribute) and isinstance(a.value, ast.Name) and a.value.id == "next":
            return a.attr, b
    raise ProbForgeError("PRISM 갱신의 좌변은 next.<변수>여야 합니다.")
