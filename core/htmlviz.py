"""HTML 리포트 공용 인터랙션 조각(D19/D15): 의존성 없는 경량 호버 툴팁.

sim·logic 백엔드의 자체 완결형 HTML이 공유한다 — 커서 위치의 값을 호버로 보여주는 작은
바닐라 JS와 그 스타일이다. Plotly 같은 외부 라이브러리·CDN을 쓰지 않아 오프라인에서 그대로
동작하고 리포트가 비대해지지 않는다(자체 완결성 유지). `data-tip` 속성을 가진 요소(막대·
히스토그램 칸·상태 카드 등)에 마우스를 올리면 그 텍스트를 커서 옆에 띄운다.
"""
