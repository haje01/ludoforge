...
# 검사(check) kind별 필요 필드(D12·D19). reachable/invariant는 `that`,
# distribution은 `expr`(수치식, sim 백엔드 전용). (PCTL `prob`은 D23으로 제거.)
_CHECK_KINDS = ("reachable", "invariant", "no_deadlock", "distribution")
...
def _parse_check(item: Any, index: int, path: Path) -> Check:
    ...
    # kind별 필요 필드 강제(D12·D19): reachable/invariant는 that, distribution은 expr.
