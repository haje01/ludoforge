...
설계 메모:
- 비-튜링완전(D21): 문법에 루프·함수·import가 없다. 표현식은 화이트리스트 노드(논리·비교·
  산술·이름·수)만 — `eval` 없이 **문자열로 lowering**해 기존 ast 화이트리스트 평가기(§7,
  D2)·Z3 번역기가 그대로 소비한다. 즉 IR의 표현식 문자열 표기는 YAML과 동일한 파이썬-식이다.
- `RANGENUM`은 소수점 뒤 숫자를 강제(`\\d+(\\.\\d+)?`)해 `0..30`이 float `0.`으로
  잘못 잠식되는 것을 막는다. 표현식 안의 수치는 별도 `NUMBER`(컨텍스트 렉서가 구분).
- `==`(비교)만 CMP 토큰에 둔다 — 단독 `=`(대입)는 전이 효과 전용(S3)이라 술어 위치에서
  쓰면 구문 오류가 된다(D21의 `=`/`==` 판별 기반).

현재 구현 범위: **S1 `domain` + S2 정적식 + S3 전이(`transitions`/`outcomes`/`pref`)**.
전이 효과는 `var' = expr`(대입) → IR의 `next.var == expr` 문자열로 lowering하고, `;` 다중
대입은 `and` 결합한다(D21). `=`(프라임 LHS 필수)/`==`(술어) 판별은 문법이 강제한다.
...
assign: NAME "'" "=" sum                 // 프라임 LHS + 단독 `=` → 다음 상태 대입
...
class _ToIR(lark.Transformer[lark.Token, RuleSet]):
    ...
    def assign(self, items: list[Any]) -> str:
        # `var' = rhs` → IR의 다음 상태 제약 문자열(다운스트림 엔진/번역기 표기와 동일).
        name_tok, rhs = items
