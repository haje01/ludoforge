...
- **프레임 = 미변경 유지**: 전이 outcome이 `next.X`로 건드리지 않은 변수는 다음 상태에서
  값이 유지된다(`next.y == y`). PRISM 갱신 의미와 일치 → ProbForge와 모델 공유.
...
속성(D12): reachable / invariant / no_deadlock. prob는 ProbForge 전용이라 건너뛴다.
...
from forge_core.ir import Rule, RuleSet
from ruleforge.solver.translator import translate_expression
...
def format_bmc_report(report: BmcReport) -> str:
    ...
    if report.skipped_prob:
        lines.append("")
        lines.append(
            "ℹ️ prob 속성은 ProbForge(PRISM) 전용이라 건너뜀: " + ", ".join(report.skipped_prob)
        )
    return "\n".join(lines)
