...
유한 상태(ProbForge/PRISM 전제, D13) 검사는 backend-agnostic `validate()`와 분리된
별도 게이트 `check_finite_state()`에 둔다 — RuleForge(Z3)는 무한 정수를 허용하므로.
...
def check_finite_state(ruleset: RuleSet) -> None:
    """ProbForge(PRISM) 전제(D13): 모든 int 변수에 유한 min·max가 있어야 한다.

    PRISM은 상태공간을 빌드하므로 경계 없는 변수를 다룰 수 없다. bool/enum은 본디
    유한하고, real은 이산화가 필요해 현 단계 ProbForge 범위 밖이다. 위반 시 SchemaError로
    친절히 안내한다.

    backend-agnostic `validate()`와 **분리된 별도 게이트**다 — RuleForge(Z3)는 무한 정수를
    허용하므로 이 검사를 적용하지 않는다. ProbForge(Phase 4)가 번역 전에 호출한다.
    """
    errors: list[str] = []
    for v in ruleset.variables:
        if v.type == "int" and (v.min is None or v.max is None):
            ...
            errors.append(
                f"변수 '{v.name}'(real): ProbForge는 실수 변수를 직접 다루지 못합니다(이산화 필요)."
            )
    if errors:
        raise SchemaError("ProbForge(유한 상태) 검증 실패:\n" + "\n".join(f"- {e}" for e in errors))
