...
- **`=`(대입) vs `==`(비교) 분리 — 엔진 의미론을 정직하게 노출.** 전이 효과는 *이미*
  ...
  - **다음상태(프라임 `gold'`) ⟺ `=`(대입)** — 전이 `then`/`outcomes`에서만. PRISM `(gold'=expr)`와 동형.
  - **같은상태(프라임 없음) ⟺ `==`(비교)** — `when`·`init`·정적 `constraint`·`check.that`.
  - 판별자가 1:1(프라임 유무). `when gold = 5`(가드 대입)·`then gold' == x`(효과 비교)는
    **파스 에러** → 오타 방어(안전성 이득, Python 직관 그대로 재사용).
  - **다중 효과는 `and`가 아니라 `;`** — 병렬 대입 집합(`{ gold' = 0; room' = hall }`).
    `대입 and 대입`은 타입 에러로 차단. PRISM 병렬 업데이트와 정합.
  - **포기:** BMC의 술어 효과(`gold' > gold` 비결정)를 막는다(sim은 이미 거부). 비결정은
...
assign        : NAME "'" "=" expr               // gold' = expr  (다음 상태 설정)
...
COMMENT : "//" /[^\n]*/                          // 줄 주석
// NAME [a-zA-Z_]\w* · NUMBER int|float · STRING "..." · ".." 범위 · "'" 프라임
```

**파스 후 정적 규칙(판별자·게이트):**
1. `=`(assign)은 전이 효과에서만, LHS는 프라임 변수. 그 외 위치 → 에러.
2. `==`/CMP는 pred 위치에서만, 프라임 변수 금지. 프라임은 assign LHS에서만.
...
- §3 EBNF를 Lark 문법으로. 토큰(프라임·`..`·`//`·`${}`)·우선순위(CMP < and/or, `&` 정밀도
