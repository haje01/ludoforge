...
> **진행:** Phase 1~2를 얇은 수직 슬라이스(domain→정적식→전이→checks→템플릿)로 합쳐
> 진행 중. 슬라이스별 상태·작업 로그는 [PROGRESS.md](PROGRESS.md). S1(domain) ✅ 완료.
...
## 7차 마일스톤 — 구조 단순화(PRISM 격하 + 던전 예제 통합) — 📋 계획·미비준 (2026-06-24)

> 설계 근거·결정은 **decisions.md D23**(D13·D16·D19 위상 갱신). 한 줄: **PRISM을 사용자
> 표면에서 내려 테스트 전용 오라클로 격하하고, 던전 예제 4벌을 하나의 실전형으로 통합**한다.
> 사용자 비준(2026-06-24): PRISM=테스트 오라클 격하 · 던전/전이 계열만 통합(정적 예제 유지).
>
> **머지 후 주석(2026-06-25, feat/new-dsl):** 6차=외부 DSL이 선점 → 본 마일스톤을 7차,
> 결정을 D21→**D23**으로 옮김. 예제가 모두 `.lf`로 이관됐으니 아래 `.rule` 경로·통합 대상은
> `.lf` 기준으로 재서술 필요(D23 비준 시). **아직 미비준 — 착수 전.**
...
- **던전 통합:** 단일 MDP+pref `dungeon.rule`(클래스 밸런스 + 욕심/안전 pref) — bmc로 건전성,
...
- `examples/dungeon_sim.rule` → `tests/fixtures/oracle_dungeon.rule`(이름 명확화). prob 검사가
  있으면 reachable로 정리. `test_sim_oracle`·`test_sim_scale` 경로 갱신.
- **성공 기준:** 오라클 회귀(PRISM 정확값 ∈ sim CI) 픽스처 경로로 통과(설치 시)·미설치 skip.
  예제 디렉토리에서 오라클 전용 파일이 빠짐.

**Phase 3 — 던전 예제 통합** *(행위적 변경, 핵심)*
- 단일 `examples/dungeon.rule` 작성: 클래스 밸런스(role sweep·win_gold 파생·전투 tables) +
  "욕심(dive/fight) vs 안전(return)" `pref` 선택. checks: bmc용(클래스별 winnable·no_deadlock·
  불변식) + sim용(직업별 승률 reachable·gold `distribution`). `dungeon_policy`·`market_sim`은
  통합 흡수 후 제거(real 쇼케이스 보존 필요성 검토 — 별도 최소 예제로 남길지 결정).
- `test_corpus`(EXAMPLE_EXPECTED)·`test_bmc`·예제 README 갱신.
- **성공 기준:** 통합 `dungeon.rule`이 `ludoforge bmc`·`ludoforge sim`로 동작(클래스별 결과·
  정책 라벨). 던전 계열 예제 4→1(+real 처리 결정). 통합 모델 동작 회귀 테스트.
...
- **`market_sim`(real) 처리:** 통합 던전에 real을 녹일지(예 gold를 real로) vs 최소 real
  쇼케이스로 남길지 — sim의 연속·고차원 능력(D19) 시연을 잃지 않게 결정.
