...
| P2 docgen+CLI | `core/docgen.py` + `ludoforge doc`(HTML/MD, 접힌 템플릿 렌더) | ⬜ | |
| P3 예제 저술·문서 정합 | dungeon.lf·dungeon_race.lf 저술 + CLAUDE §4·README | ⬜ | |

13·14차는 P0(비준)부터 — 착수 전.

| 마일스톤 | 내용 | 상태 |
|----------|------|------|
| 13차 주사위 확률식 | `chance(2d6 >= …)`/`rest` → Fraction 닫힌형 → float weight | ⬜ P0 비준 대기 |
