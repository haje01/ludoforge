...
disambiguation: **문맥 기반**(사용자 결정). 근거는 [PLAN.md](PLAN.md) 및 decisions.md D8 참조.

| 단계 | 내용 | 상태 | 비고 |
|------|------|------|------|
| E1 | (행위) enum→EnumSort + 비교 문맥 해석 + 교차 enum 에러 | ✅ | translator 중심, 81건 |
| E2 | (행위) checks 회귀 + 중복 값 이름 코퍼스 | ✅ | enum_fix는 const==const, 83건 |
| E3 | (문서) decisions.md D8, CLAUDE.md §4, examples, README | ✅ | |

상태 범례: ⬜ 대기 / 🔵 진행중 / ✅ 완료 / ⚠️ 막힘

다음 후보는 [PLAN.md](PLAN.md) "2차 후보" 참조(Real 범위 도달성/경계 검사 확장/CI/expect: 등).
...
- 2026-06-16: 2차 D8(enum EnumSort 고도화) 완료. 정수 인코딩→z3 EnumSort. 서로 다른 enum이
  ...
  examples/day_night_cycle.rule. 테스트 83건, ruff/format/mypy(strict) 통과.
