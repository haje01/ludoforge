...
| 단계 | 내용 | 상태 | 비고 |
|------|------|------|------|
| F1~F3 | real 끝점 검사 + BoundUnreachable + 문서 | ✅ | 91건, 커밋 d5e9873 |

## 2차 — 명시적 도달성 단언 (`expect:`, D10) — ✅ 완료

자동 도달성 추론(D3)의 역방향. 최상위 `expects:` 섹션, `rules ∧ that` SAT 여부로 판정.
근거는 [PLAN.md](PLAN.md) 및 decisions.md D10 참조.

| 단계 | 내용 | 상태 | 비고 |
|------|------|------|------|
| G1 | (행위) IR Expect + 로더 expects 파싱 + 스키마(중복 id·참조) | ✅ | 95건 |
| G2 | (행위) translator expect_constraints + checks UnmetExpectation + report | ✅ | _feasibility 재사용, 97건 |
| G3 | (문서) decisions.md D10, CLAUDE.md §4, examples, README | ✅ | |

상태 범례: ⬜ 대기 / 🔵 진행중 / ✅ 완료 / ⚠️ 막힘

다음 후보는 [PLAN.md](PLAN.md) "2차 후보" 참조(Real 완전 Optimize A-ii/경계 검사 확장/CI 통합).
...
- 2026-06-16: 2차 D9(Real 끝점 도달성) 완료. real 변수의 선언 min/max 끝점 도달성 검사
  ...
  ruff/format/mypy(strict) 통과. (커밋 d5e9873)
- 2026-06-16: 2차 D10(명시적 도달성 단언 expect:) 완료. 최상위 expects: 섹션, IR Expect +
  로더/스키마/translator(expect_constraints)/checks(_check_expects, UnmetExpectation)/report.
  domain∧rules∧that가 unsat이면 미충족 모순, 범인 룰 unsat_core. 변수별 경계로는 안 보이는
  조합 도달성 검증 — examples/stat_budget.rule. 테스트 97건, ruff/format/mypy(strict) 통과.
