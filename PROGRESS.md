...
## 규칙서 SSOT 아크 (12~14차) — ✅ 완료 (2026-07-06)
...
### 14차 마일스톤 — ghost 서술 변수 (D31) — ✅ 완료 (2026-07-06)

| Phase | 내용 | 상태 | 비고 |
|-------|------|------|------|
| P0 D31 기록·비준 | 단방향 의존·init 상수 고정·erase는 core·리포트 라벨 | ✅ | 2026-07-06 사용자 비준 |
| P1 문법·IR·게이트 | `ghost` 수식어·`Variable.ghost`·`_check_ghost_one_way` | ✅ | 위반 8종 거부·배열 승계·골든 무회귀 |
| P2 erase+배선 | `core/ghost.py`·bmc/PRISM erase·sim 원본+라벨 | ✅ | 픽스처 쌍: bmc 지위 동일·sim 비트 동일·분포 신규 |
| P3 예제·문서 | dungeon `fights`+`fight_count`·docgen 표기·CLAUDE §4.5 | ✅ | bmc 9검사 불변+각주·직업별 전투 횟수 분포 |

### 규칙서 SSOT 아크(12~14차) 마감 요약 (2026-07-06)

진단(형식화 손실/서술 손실) 후 한 아크로 진행: **12차** 문서 메타데이터+`ludoforge doc`
(D29 — `.lf`가 곧 규칙서, `[[이름]]` 참조 게이트) → **13차** 주사위 닫힌형 `chance`/`rest`
(D30 — 룰북 원형 수치가 SSOT에, 매직 넘버 표 제거) → **14차** `ghost` 서술 변수(D31 —
서술 지표를 검증 부하 0으로). 방법론은 기존 두 계보의 확장으로 일관: **"지워지는 주석"**
(D12→D20→D27→D29·D31)과 **"순수 desugar"**(D18→D28→D30). 세 백엔드·결정론 경계는 아크
전체에서 무회귀(골든·픽스처 쌍·PRISM 오라클 실측으로 증명). `examples/dungeon.lf` 하나가
검증 모델이자 규칙서 SSOT로 완성 — `ludoforge doc`이 사람용 규칙서를, bmc가 증명을,
sim이 추정(전투 노출량 분포 포함)을 낸다. 후속 후보는 PLAN "보류 중"(맵/페이즈 desugar·
concepts.md 아크 개념 절 등 — 트리거 명시).

### 15차 마일스톤 — 도메인 축소 피벗: 수치·경제 시스템 검증기 (D32) — 진행 중
...
- 2026-07-07: **15차 P0~P2 완료 — 도메인 축소 피벗(D32).** ① 진단 비준: 전체 게임 기술
  ...
  프론트엔드 제거: `core/loader.py`는 진입점(IO·병합·`.rule` 명시 거부)만 남김(552→95줄),
  픽스처 19개 `.lf` 이관(제거 직전 IR 등가 19/19 실측), old_examples/·test_expand.py·YAML
  골든 등가 하니스 은퇴(접힘 골든은 race_manual.lf 쌍이 유지), test_transitions 인라인
  YAML → .lf 재작성(음수 pref/weight 거부는 스키마 책임으로 단정 이동), pyyaml 의존 제거.
  ③ 문서: CLAUDE §1(정의·해결 문제·비목표·인터페이스 방향)·§3~6(.rule 흔적), README
  (포지셔닝·범위 문단), rules/README, D32 ADR. 전체 367 통과(+5 skip), ruff/mypy(strict)
  clean. 다음=P3(웹 MVP — 시트 임포터·LLM 번역 수리 루프·FastAPI 셸).
...
- 2026-07-06: **14차 완료 — ghost 서술 변수(D31, P0~P3) · 규칙서 SSOT 아크 마감.**
  ...
  ② 게이트(`_check_ghost_one_way`): ghost 읽기 허용 = ghost 대입 RHS·distribution expr뿐
  — 가드·constraint·expects·reachable/invariant that·pref/weight·비-ghost 효과 RHS 참조
  거부, init은 `ghost == 상수` 고정 필수(파생·미고정·자유 sweep 거부). ③ `core/ghost.py`
  `erase_ghosts`(순수 IR→IR): 선언·대입·init conjunct 제거, 전부-ghost 효과는 `True`(항등
  — 분기 구조 보존, PRISM은 `true` 갱신). bmc(`run_bmc`)·PRISM(`generate`)이 소비 전
  erase — BmcReport.erased_ghosts 각주, `check_finite_state`는 ghost 스킵(무한 int 허용).
  sim은 원본 실행 + DistributionResult.ghost_expr 라벨("서술 변수 — 논리 검증 제외",
  텍스트·HTML). ④ 핵심 회귀(픽스처 쌍 ghost_counter ↔ plain): bmc 검사 지위 동일,
  sim 비-ghost ProportionResult **완전 동등(비트 동일 — ghost 대입은 rng 미소비)**,
  ghost 분포 신규. ⑤ 예제: dungeon.lf `ghost fights`(전투 효과에 병기) + `fight_count`
  distribution — bmc 9검사 지위 불변(각주만 추가), sim이 직업별 전투 노출량 분포를 냄
  (rogue ~6.8 vs cleric ~5.2). 문서: CLAUDE §4.5·§6, README, examples README(concepts는
  후속 — 아크 개념 절 일괄 저술 트리거). 테스트 14건, 전체 392 통과, ruff/format/mypy
  clean. **규칙서 SSOT 아크(12~14차) 종결.**
