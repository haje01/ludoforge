...
> **결정론적으로 증명**하고(RuleForge 백엔드), 확률은 PRISM으로 **계산**한다(ProbForge 백엔드).
...
- **확률 검사(ProbForge/PRISM)**: 같은 전이 시스템을 PRISM 확률 모델로 번역해 승리
  확률·기대값 등 PCTL 속성을 검사(`ludoforge prob`, PRISM 설치 시).
...
> 참고: 저장소 URL(`haje01/ruleforge.git`)은 GitHub 리포지토리 이름이라 그대로 둔다.
> 리포를 `ludoforge`로 개명하면 이 URL도 함께 바꾼다.
...
상태 도달성, 불변식 유지, 데드락을 k 스텝까지 본다(반례 경로 동봉). 던전!(WotC)
미니 예제로 확인:

```bash
ludoforge bmc examples/dungeon.rule --k 10
```

```text
[1] 속성 'winnable' (reachable) — ...: ✅ 도달 가능 (깊이 4)
    경로:
      s0: gold=0, room=center, role=fighter, win_gold=10000
        --[descend_to_l1]-->
      ... (중략) ...
      s4: gold=10000, room=center, role=fighter, win_gold=10000
```
...
```bash
ludoforge prob examples/dungeon.rule          # PRISM 모델 생성 + (설치 시) 실행
```
...
> **3차**(다중 백엔드, 완료): 공유 IR(`forge_core`) 위에 전이 시스템(init/transitions/
> properties, D12)을 두고, **RuleForge**(Z3/BMC — 도달성·불변식·데드락, D15)와
> **ProbForge**(PRISM — 확률·PCTL, D16) 두 백엔드로 동역학을 검사한다. 보드게임
