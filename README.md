...
- **웹 인터페이스(`ludoforge web`, D32)**: 기획자가 DSL을 배우지 않아도 되는 표면 —
  산문/시트(CSV)를 넣으면 **AI가 `.lf`로 번역**(로더·스키마 오류를 되먹이는 수리 루프)하고,
  기획자는 **규칙서(산문) 미리보기를 읽고 승인**한 뒤 check/bmc/sim을 실행한다. 시트는
  LLM 없이 `table` 절로 결정론 변환. 판정은 항상 solver — 자세한 내용은
...
종료 상태에서 식 값을 모아 평균·신뢰구간·백분위로 추정한다(sim 전용). **`ghost` 서술
변수**(D31: `ghost fights: int 0..`)는 sim만 실행하는 검증 제외 상태다 — 전투 횟수·게임
길이 같은 서술 지표를 BMC/PRISM 상태공간 부하 0으로 얻는다(bmc는 제거를 각주로 명시,
distribution 결과엔 "서술 변수 — 논리 검증 제외" 라벨). **`--html <경로>`**
