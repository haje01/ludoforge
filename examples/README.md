...
| [`dungeon_race.lf`](dungeon_race.lf) | (전이 시스템, 2인 레이스) | 던전! **2인 레이스판**(북극성 2단계, D27) — 턴 교대·**전이 소유 태그(`player`)**·공유 몬스터 덱(비복원)·비대칭 정책(욕심 p1 vs 안전 p2). `bmc`로 양쪽 승리 가능성·건전성 증명, `sim`으로 **매치업 승률**(욕심 ~67% vs 안전 ~33%) 추정 |
