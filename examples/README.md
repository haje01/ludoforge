...
실제 게임 기획에서 나올 법한 모순을 담은 예제다. RuleForge가 잡아내는 세 가지
모순 유형(범위 봉쇄 / enum 도달 불가 / 전역 over-constraint)과 정합 케이스를 보여준다.
