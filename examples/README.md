...
| [`dungeon_race.lf`](dungeon_race.lf) | (전이 시스템, 2인 레이스) | 던전! **2인 레이스판**(북극성 3단계, D27·D28) — **배열(`gold[p1, p2]`) + `for` 템플릿 한 벌**로 접힌 다인 모델: 턴 교대·**전이 소유 태그(`player`)**·공유 몬스터 덱(비복원)·비대칭 정책(욕심 p1 vs 안전 p2, 정책 표). `bmc`로 양쪽 승리 가능성·건전성 증명, `sim`으로 **매치업 승률**(욕심 ~67% vs 안전 ~33%) 추정. 플레이어 추가 = 값 목록·표 한 곳 수정 |
| [`market_sim.lf`](market_sim.lf) | (전이 시스템, real) | 두 자산을 복리로 굴리는 **연속(real)·다변수** 모델 — `sim`이 분포로 추정(PRISM이 못 다루는 영역, D19) |
| [`terraforming_mars.lf`](terraforming_mars.lf) | (전이 시스템, 경제) | 테라포밍 마즈의 **수치 서브시스템**(글로벌 파라미터·TR·세대 소득·표준 프로젝트 비용 — D32 범위 시연). 카드 메커니즘은 비목표. `bmc`가 TR 불변식·무데드락을 k-귀납 증명하고 완주 도달성 증인(깊이 43)을 내며, `sim`이 게임 길이(~49세대)·종료 잔고 분포를 추정. 웹 인터페이스 기획 입력 예제([산문+시트](terraforming_mars.prose.md))와 짝 |
...
**클래스의존 전투**(같은 몬스터라도 클래스별 격파 목표값이 다름 — `chance(2d6 >= beat[mon][cls])`/`rest`가 닫힌형 확률로 desugar, D30 — 모델엔 룰북 원형 수치만 남는다),
