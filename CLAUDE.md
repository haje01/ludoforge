...
**이름:** Ludoforge — 게임 기획 검증 툴킷. (논리 백엔드 `logic/`·Z3·BMC,
확률 백엔드 `prob/`·PRISM, 공유 프론트엔드 `core/`. 모두 우산 CLI `ludoforge`의
서브명령으로 노출된다.)
...
- 밸런스의 *재미·튜닝* 평가(연속적 품질, "A직업 승률이 B와 5% 이내인가" 류)는 다루지
  ...
  없는가 · 기대 게임 길이가 유한한가" 같은 정량 *건전성* 속성은 확률 백엔드의
...
공유 IR(`core`)이 이를 표현하고, 논리 백엔드(Z3/BMC)와 확률 백엔드(PRISM)가 같은
...
checks:                                 # 질의. kind로 백엔드 공통 의미 표현
  - { id: winnable, kind: reachable, that: "gold >= 10000 and room == center" }
  - { id: gold_ok,  kind: invariant, that: "gold >= 0" }
  - { id: likely,   kind: prob, spec: "P>=0.95 [ F (room == center) ]" }  # 확률 백엔드 전용
```

규칙:
- `next.<var>`(다음 상태 참조)는 **전이 then에서만** 허용. constraints·init·expects·check.that
...
- `checks`의 `kind` ∈ {`reachable`, `invariant`, `prob`, `no_deadlock`}. `reachable`/
  `invariant`는 `that`(상태 술어), `prob`는 `spec`(PCTL 문자열, 확률 백엔드 전용 — core는
  구문 검사 안 함). 질의 dialect는 백엔드별로 가른다(D11).
- 유한 상태(확률 백엔드/PRISM 전제, D13)는 `validate()`와 분리된 `check_finite_state()`가
...
`constraints`/`transitions`/`checks`의 각 항목은 `for:` 템플릿으로 쓸 수 있고, 공통 데이터는
최상위 `tables:`에 표로 둔다. **로더가 파싱 직후 구체 항목으로 펼친다(desugar)** — IR·번역기·
BMC·PRISM은 그대로 구체 항목만 본다. 펼치기는 순수 구문 변환이라 결정론 경계를 건드리지 않는다(§2).

```yaml
tables:                                # Tier 2: 공통 데이터 표(desugar 시점, IR엔 안 들어감)
...
transitions:
  - id: "fight_${monster}_${cls}"
    for: { monster: [goblin, dragon], cls: [fighter, rogue] }   # 데카르트 곱
    when: "role == ${cls} and monster == ${monster}"
    outcomes:
      - { weight: "${win[monster][cls]}", then: "next.gold == gold + ${reward[monster]}" }
```

- `for:` 형태 두 가지:
  - **매핑 `{param: [값들]}`**: 값들의 **데카르트 곱**(product). (위 예)
  - **레코드 리스트** `[{...}, ...]`: 행마다 바인딩. 표 없이 행에 데이터를 직접 둘 때.
- `${expr}` 치환: `expr`는 파라미터/`tables` 이름과 색인(`win[monster][cls]`)을 쓰는 **제한된
  식**이다. ast로 파싱해 화이트리스트 노드(Name·Subscript·Constant)만 평가한다 — `eval`은 쓰지
  않는다(§7, 번역기와 같은 규율). 미정의 이름·색인 실패는 LoaderError(위치 보고).
- 문자열 **전체가 `${expr}`이면 값의 타입을 보존**(weight 숫자·정수), 아니면 문자열
  보간(`gold <= ${cap[monster]}` → `gold <= 20`).
- 생성 항목의 `id`는 템플릿대로 결정적·추적 가능해야 한다(`fight_dragon_rogue`) — unsat
  core·BMC 반례 리포트가 여전히 사람이 읽는 범인을 짚게 하기 위함(원칙 4).
...
```
core/          # 공유 DSL 프론트엔드(SSOT) — 두 백엔드가 같은 IR을 소비
  ir.py              # 중간표현 데이터클래스 (전이 시스템 포함)
  loader.py          # .rule 파일 → 내부 IR
  schema.py          # 스키마·참조 검증 + check_finite_state(확률 백엔드 게이트)
