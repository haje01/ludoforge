...
### 4.2 템플릿 확장 — `for:` / `${param}` (Tier 1, D18)

클래스×몬스터처럼 구조가 같고 데이터만 다른 항목이 곱으로 늘어나는 걸 막기 위해,
`constraints`/`transitions`/`checks`의 각 항목은 `for:` 템플릿으로 쓸 수 있다. **로더가
파싱 직후 구체 항목으로 펼친다(desugar)** — IR·번역기·BMC·PRISM은 그대로 구체 항목만
본다. 펼치기는 순수 구문 변환이라 결정론 경계를 건드리지 않는다(§2).

```yaml
transitions:
  - id: "fight_${monster}_${cls}"
    for:                               # ① 레코드 리스트: 행마다 다른 데이터
      - { monster: goblin, cls: fighter, win: 0.92, miss: 0.07, death: 0.01, reward: 2 }
      - { monster: dragon, cls: rogue,   win: 0.08, miss: 0.84, death: 0.08, reward: 10 }
    when: "role == ${cls} and monster == ${monster}"
    outcomes:
      - { weight: "${win}",  then: "next.gold == gold + ${reward}" }   # 전체가 ${win} → 숫자 보존
      - { weight: "${miss}", then: "..." }
```

- `for:` 형태 두 가지:
  - **레코드 리스트**: 행마다 바인딩. 행별로 다른 값(승률 등 2D 데이터)에 쓴다.
  - **매핑 `{param: [값들]}`**: 값들의 **데카르트 곱**(product). 본문이 균일할 때.
- 치환 `${name}`: 문자열 **전체가 `${name}`이면 레코드 값의 타입을 보존**(weight 숫자,
  정수 등), 아니면 문자열 보간(`gold <= ${cap}` → `gold <= 20`). 미정의 파라미터는 오류.
...
- **주의:** 펼치기는 *소스*를 줄일 뿐 *검증 모델*은 줄이지 않는다 — PRISM/Z3엔 여전히
  N·M개 명령이 간다(상태기계 본질). 데이터(N·M개 가중치)도 그대로 남는다.
