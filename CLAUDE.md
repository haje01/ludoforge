...
transitions:                            # 상태 → 다음 상태. next.<var>로 다음값 참조
  ...
  - { id: dive,  when: "room == l2", pref: 0.3, then: "next.room == l3" }   # 욕심
  - { id: leave, when: "room == l2", pref: 0.7, then: "next.room == center" }  # 안전
...
checks:                                 # 질의. kind로 백엔드 공통 의미 표현
  ...
  - { id: gold_dist, kind: distribution, expr: "gold" }   # sim 전용 — 종료 상태 분포 추정(D19)
```
...
- 전이 `pref`는 **플레이어 선택**의 상대 가중치다(sim 전용, D20). 한 상태에서 여러 전이가
  ...
  - 결과는 **"주어진 정책 하의 추정 · Pmax 아님"**으로 라벨한다 — 고정 정책의 값은 Pmax의
    하한이다(정책이 우연히 최적일 때만 일치). 예제: `examples/dungeon_policy.rule`(욕심 vs 안전).
