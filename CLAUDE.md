...
> *이미* 대입+프레임이므로(전이 `then` → `next.X` 배정·미언급 변수 유지, D15·§4.1) 효과
> (`then`/`outcomes`)엔 `=`(대입; `then` 문맥이 곧 다음상태), 같은상태 술어(`when`·`init`·정적
> `constraint`·`check`)엔 `==`. (다음상태 마커 프라임 `var'`은 D22로 제거 — `then` 문맥이 잉여로 만듦.)
